...
wget "$v2_playlist_url" -O "test_audio.json"

JSON="test_audio.json"
...
printf "%s" "$raw_init_segment" > audio_init_segment
...
' | while read -r url filename; do
    ...
    if [ -f "./audio/$filename" ]; then
      continue
    fi

    curl -L -# -f -o "./audio/$filename" "${base_media_url}${url}"
